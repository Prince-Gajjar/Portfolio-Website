...
export function setCharTimeline(
  ...
  character?.children.forEach((object: any) => {
    ...
    if (object.name === "screenlight") {
      ...
      object.material.emissive.set("#b5e252");
      pulseTimeline = gsap.timeline({ repeat: -1, repeatRefresh: true }).to(object.material, {
  ...
  // Return cleanup function to destroy all anims/ScrollTriggers/intervals
  return () => {
    ...
    ScrollTrigger.getById("char-tM2")?.kill();
  };
}
...
export function setAllTimeline() {
  ...
  const careerTimeline = gsap.timeline({
    scrollTrigger: {
      ...
      end: isMobile ? "bottom 95%" : "bottom 30%",
  ...
  // Return cleanup function to destroy career timelines and triggers
  return () => {
    careerTimeline.kill();
